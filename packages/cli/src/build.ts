...
import { wrapDocument, type Heading, type BlockCategory } from '@rundocs/renderer-html';
...
export async function build(resolved: ResolvedInput, outDirName = 'dist'): Promise<BuildResult> {
  ...
  for (const file of files.sort()) {
    ...
    const blockCategories = (vfile.data as { blockCategories?: BlockCategory[] }).blockCategories ?? [];
    const rel = relative(workspaceRoot, file).replace(/\\/g, '/');
    const outRel = rel.replace(/\.md$/, '.html');
    const outFile = join(outDir, outRel);

    await mkdir(dirname(outFile), { recursive: true });
    await writeFile(outFile, wrapDocument(bodyHtml, rel, headings, blockCategories), 'utf8');
